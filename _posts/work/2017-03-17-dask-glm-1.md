...
[dask-glm](https://github.com/dask/dask-glm) library.  We also talk about the
experience of learning Dask to do this kind of work.
...
TODO: matt needs to change this language

1.  Use parallel multi-dimensional arrays to contruct algorithms from common operations like matrix multiplication, SVD, etc..  This mirrors mathematical algorithms well but lacks some flexibility.
...
Let's build up an example of fitting a large linear regression model using both built-in array parallellism and fancier, more customized parallelization features that Dask offers.  The [dask.array](http://dask.pydata.org/en/latest/array.html) module helps us to easily parallelize standard NumPy functionality using the same syntax -- we'll start there.
...
<img src="{{ BASE_PATH }}/images/X-white.svg"
     width="80%"
     alt="">
...
It's worth noting that almost all of this code is exactly the same as the equivalent NumPy code.  Because Dask.array and NumPy share the same API it's pretty easy for people who are already comfortable with NumPy to get started with distributed algorithms right away.  The only thing we had to change was how we produce our original data (`da.random.normal` instead of `np.random.normal`) and the call to `dask.compute` at the end of the update state.  The `dask.compute` call tells Dask to go ahead and actually evalute eveyrthing we've told it to do so far (Dask is lazy by default).  Otherwise, all of the mathematical operations, matrix multiplies, slicing, etc. are exactly the same as with Numpy, except that Dask.array builds up a chunk-wise parallel computation for us and Dask.distributed can execute that computation in parallel.
...
<img src="{{ BASE_PATH }}/images/grad_step-white.svg"
     width="60%"
...
Now that we've seen how to use the built-in parallel algorithms offered by Dask.array, let's go one step further and talk about writing more customized parallel algorithms.  Many distributed "consensus" based algorithms in machine learning are based on the idea that each chunk of data can be processed independently in parallel, and send their guess for the optimal parameter value to some master node.  The master then computes a *consensus* estimate for the optimal parameters and reports that back to all of the workers.  Each worker then processes their chunk of data given this new information, and the process continues until convergence.  From a parallel computing perpsective this is a pretty simple procedure, it's just simple map-reduce.  We'll use this as a very simple example for how to use Dask's more customizable parallel options.
...
There is of course a little bit more work occuring in the above algorithm, but it should be clear that the distributed operations are *not* one of the difficult pieces.  Using dask.delayed we were able to express a simple map-reduce algorithm like ADMM with similarly simple Python for loops and delayed function calls.  Dask.delayed is keeping track of all of the function calls we wanted to make and what other function calls they depend on.  For example all of the `local_update` calls can happen independent of each other, but the concensus comptuation blocks on all of them.


We hope that both parallel algorithms shown above (gradient descent, ADMM) were
striaghtforward to someone reading with an optimization background.  These
...
optimization perspective (stopping criteria, step size, asynchronicity, etc.).
...
and ADMM alongside Scikit-Learn's LogisiticRegression and SGD implementations
...
You can see the difference in complexity of the algorithms.  ADMM is much
simpler from Dask's perspective but also saturates hardware better for this
chunksize.

#### Profile Plot for Proximal Gradient Descent

<iframe src="https://cdn.rawgit.com/mrocklin/5dec93966e0daa2edb8fcde8449a5335/raw/52f81540fd969e3ff4c69f1f44a401d25d9134ff/dask-glm-proximal-grad-small.html"
...
1. Creating distributed algorithms with Dask was surprisingly easy; there is
   ...
   `compute`, `rebalance`, etc. but that can't be avoided.  Using Dask for
...
2. Getting the particulars of algorithms correct is non-trivial; there is still
   ...
   tradeoffs that are occuring in many of these algorithms, as well as
