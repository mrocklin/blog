...
1. easy to write and play with; want to play with even more algorithms (especially consensus sharing)
    great learning environment for distributed algorithms and the unique challenges
    associated with them (communication costs, for example)
2. would like to better understand any tolerance vs. accuracy tradeoffs that are occuring
3. testing optimization algorithms is hard! related to the above
4. writing algorithms in isolation is not nearly as fun as writing algorithms with collaborators! plz join me! 
5. there is a lot of misunderstanding in the community about what algorithms do what things / solve what
    problems -- for example, what different regularizations are trying to accomplish 
    the goal is that our API exposes these subtle distinctions more transparently
...
2.  User APIs (link to issue)
3.  Extend GLM families
4.  Write more extensive rigorous algorithm testing (two issues to link to)
5.  Begin work on smart initialization routines (link to issue)
