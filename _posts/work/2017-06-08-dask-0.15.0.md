...
changes.  This blogpost outlines notable changes and changes since the last
release on May 5th.
...
Thanks to recent changes in NumPy, NumPy ufuncs now operate as Dask.array
ufuncs.  Previously they would convert their arguments into Numpy arrays and
then operate concretely.
...
dask.arrays.  This makes it easier to write code that seemlessly works with
either array type.
...
like web backends or when building some more advanced algorithms in machine
...
These methods are still around, but starting has changed and we now recommend
using the fully public methods even in asynchronous situations (these used to
block).
...
`dask-worker ... --local-directory` option, pointing to some other better
place, perhaps a local SSD drive called `/scratch`.  Previously Dask would dump
files into the provided directory.  Now it will create a new subdirectory and
place files there.  This tends to be much more convenient for users on network
file systems.
