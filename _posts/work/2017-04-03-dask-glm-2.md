...
Now we alter this algorithm to update asynchronously, so that we our parameters
...
We define local functions for ADMM.  These correspond to solving normal Linear
regression (TODO: verify by Chris)
...
```python
# Submit starting tasks to occupy our current workers
...
while time() - start < MAX_TIME:
    ...
    # Update beta (this could be made more efficient)
    ztilde = np.mean(betas + np.array(u), axis=0)
...
can actually be the limiting factor.  We profiled and found that this was
mostly bound by computing our performance metrics (so not actually a limitation
in practice) but we felt that it was good practice to ensure that we won't be
bound by this in the future at larger scales, so now we build in batching.
...
bottlenecking client.  We just smoothly shift back over to synchronous ADMM
when the client becomes too overwhelmed.  (though again, at this scale we're
fine).

Conveniently, the `as_completed` iterator has a batches method that returns all
of the results that have come in so far.
