...
for X, y in fit_data(dfs):
    est.partial_fit(X, y)
```

This requires reading in the entire the entire dataset on one machine,
and keeps the dataset and the model on one machine. It explicitly moves
each dataset to the model.
...
is efficient. It moves the estimator to the data, instead of moving the data
to the estimator. This is much quicker: Dask chooses whether the estimator or
dataset is cheaper to move (though it's almost always the estimator).
