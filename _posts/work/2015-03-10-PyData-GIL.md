...
**tl;dr** Many PyData projects release the GIL.

Introduction
------------

The Global Interpreter Lock (GIL) stops multiple threads from simultaneously
manipulating Python objects in the same process.  This cripples *Pure Python*
shared-memory parallelism.  How much should the PyData ecosystem care about the
GIL?  My answer: *Not much*.

To be clear,i


People make a big deal out of the GIL; they
shouldn't.  The GIL is not a major issue.

We can still execute in parallel by using different
processes (e.g. `multiprocessing`).  This works fine for bulk processing of
data but fails for more complex workflows like fancy machine learning
techniques.




Moore's law is alive and well.  Transistor density continues to increase
exponentially with time, it just does so by making more cores rather than
faster clock cycles.

It may surprise you to learn that Python has a great relationship with
parallelism.  When it comes to scientific Python code it is trivial to use your
entire computer with multi-threading; the GIL is not an issue.


The Global Interpreter Lock (GIL)
---------------------------------

No discussion around parallel processing in Python is complete with addressing
the Global Interpreter Lock (GIL).  The GIL stops any two threads from
accessing Python objects at the same time.  This effectively limits Pure Python
code to a single core; a concerning limitation as our computers grow more
cores.  Generally people use `multiprocessing` in this situation (see [`toolz`
docs on `pmap`](http://toolz.readthedocs.org/en/latest/parallelism.html)) but
this forces us to copy data between processes which can slow down some
workflows.


Many Numeric Python Projects Release the GIL
--------------------------------------------

The scientific Python stack spends most of its time in C, Fortran, or Cython
code.  We spend very little time actually in Python.  The following projects
...
Computers today contain

A lot of my work pushes Python to use your entire computer.
